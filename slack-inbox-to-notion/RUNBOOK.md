...
代わりに、**Slackリンクの完全一致**で確認します。これは重複判定とまったく同じ条件なので、
判定基準がコード側と一致します。
...
後日の実データで、新規に登録された `Slack｜` ページは **INBOX のまま残る**ことを確認済みです。
...
`testFindBySlackLink` は重複判定と同じ検索条件を使うので、
**判定基準がコード側と完全に一致します。** ステータスが `INBOX` でない場合は
警告も出ます。
