...
**注意(スレッド関連メッセージ):** 保存される Slackリンクには `?thread_ts=…&cid=…` が
付くことがある(GAS は `chat.getPermalink` が返す URL をそのまま保存するため)。
Slack の「リンクをコピー」でクエリの付かない URL を渡すと、`notion:check` の
**完全一致が外れて「未登録」と偽表示される**ことがある。その場合は
**メッセージID(`p…` の数字)部分の一致**で確認する
(例: Notion の `Slackリンク` が `1784869802122299` を含む、で検索)。
