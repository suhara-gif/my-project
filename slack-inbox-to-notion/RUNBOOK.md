...
## ⚠ 既知の問題：登録直後にステータスが「完了」に変わる

**この連携が作ったページが、登録直後に別の何かによって「完了」へ書き換えられています。**
2026-07-31 時点で、`Slack｜` で始まるページ **4件すべて**が完了になっている一方、
手書き名のタスクは INBOX のまま残っていることを確認しました。

**このスクリプトはページを作成するだけで、作成後に更新することは一切ありません**
(Notion への呼び出しは検索 `databases/query` と作成 `pages` の2つだけで、
更新の経路がコード上に存在しません)。書き換えているのは別の主体です。

影響は2つ:

1. **「INBOX に溜めて朝レビューで仕分ける」という運用が成立しません。**
   これは機能の目的そのものなので、原因を止めない限りこの仕組みは用をなしません。
2. **INBOX ビューの件数を、連携の成否判定に使えません。**
   「INBOX に無い = 登録されていない」ではありません(実際に一度誤判定しました)。

### 犯人の探し方

1. タスクDB を開き、右上の **⚡(オートメーション)** に、
   ページ作成をトリガにステータスを変える設定が無いか確認する
2. 他の Claude セッション / エージェント(「タスク管理の最適化」等)に、
   タスクDB を定期的に整理する常駐処理が無いか確認する
3. 当該ページの **更新履歴**で最終編集者と時刻を見る
   (Notion は自動化による編集も実行ユーザー名義で記録するため、
   画面からは手動か自動かの区別が付きません)

### 登録できたかの正しい確認方法

**INBOX の件数を見ないでください。** 代わりに:

- GAS エディタで **`testFindBySlackLink`** を実行する
  (`TEST_MESSAGE_URL` に対象メッセージの permalink を入れておく)
- または Notion 全体検索で **Slackリンクの完全一致**でページを探す
