...
**これ1つです。** push → 既存デプロイを新バージョンに更新(**URL 不変**)→ 配信中のコードを検証、
までをまとめて行います。Slack 側の再設定は不要です。

### ⚠ `npm run push` だけでは本番は変わりません

**GAS で最も踏みやすい罠です。実際に踏みました。**

`clasp push` が更新するのは**エディタで見える HEAD** だけで、**Slack が叩く Web App は
デプロイ時点のバージョンに固定されたまま**動き続けます。つまり:

```
コードを直す → npm run push → Slack から叩く → 直っていない
                                                  ↑ デプロイが古いまま
```

この状態は、GAS の実行ログが読めない環境(下記)では切り分けが非常に困難です。
**コードを変えたら `npm run release`** と覚えてください。

| 変更したもの | 必要な操作 |
|---|---|
| **コード**(`.gs`) | **`npm run release`**(デプロイ更新が要る) |
| スクリプト プロパティのみ | `npm run props:push` → エディタで関数実行(デプロイ不要) |
...
デプロイ済みの `doGet` が返す `SCRIPT_VERSION` を、ローカルの値と突き合わせます。
...
**「直したはずなのに動かない」と思ったら、まずこれを実行してください。**
`slack_inbox_to_notion.gs` の `SCRIPT_VERSION` は、振る舞いを変えたら上げること。

<details>
<summary>手動でデプロイする場合(GAS エディタ)</summary>

**デプロイ → デプロイを管理 → 鉛筆アイコン → バージョン「新バージョン」→ デプロイ**

「**新しいデプロイ**」を押すと**URL が変わり**、Slack の Request URL の貼り直しが必要になります。
既存デプロイの編集なら URL は変わりません。
</details>

### ⚠ doPost の実行ログが読めない場合

GAS プロジェクトが「デフォルトの GCP プロジェクト」のままだと、**Web App 実行(`doPost`)の
ログが「実行数」画面で展開できません**(エディタから実行した分だけ読めます)。
この状態だと `skip:` の理由も例外も追えず、切り分けが実測頼みになります。

解消するには **プロジェクトの設定 → Google Cloud Platform プロジェクト → 標準の GCP
プロジェクトに切り替え**、Cloud Logging でログを見ます。
切り替えないうちは `npm run verify:deployed` と `testConfig` が主な診断手段です。
...
- 押せる人は `ALLOWED_SLACK_USER_ID` の 1 名だけです(複数にするならカンマ区切り対応の改修が必要。
  ここが実質の安全弁なので慎重に)。
