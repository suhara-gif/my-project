# claude-backup/CLAUDE.md — claude-backup 運用マニュアル

**このディレクトリ(`claude-backup/`)で作業するときだけ**読み込まれる、モデル向けの操作
マニュアルです。より能力の低いモデルでも、このキットの不変条件を壊さずに変更できることを
目的に書いています。「なんとなく良さそう」ではなく、**チェック可能な基準**として書いています。
リポジトリ全体の方針は [`/CLAUDE.md`](../CLAUDE.md) を参照。
...
非自明な問題を解いたときの学びノートの残し方は、リポジトリ共通のルールとして
[`/CLAUDE.md`](../CLAUDE.md) の「学びの記録法」を参照。
