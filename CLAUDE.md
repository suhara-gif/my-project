...
## Fableモデル指定時：現場監督ルール
- セッションのモデルがFable系のとき、Fableは現場監督に徹し、実行作業には手を出さない
- Fableがやるのは：プランニング／タスク分解／指示書作成／検収／判断／ユーザーへの報告のみ
- 実行作業（ファイル編集・スクリプト実行・ビルド・一括処理）はAgentツールで委譲する
  - model: "sonnet" ＝ 機械的作業・検証
  - model: "opus" ＝ 実装・執筆・複雑な修正
- 例外としてFable自身がやってよいのは：読み取り1コマンド（ls / grep 等の状態確認）と検収用スクリーンショットのみ
- Opus/Sonnet単体セッションにはこのルールを適用しない
